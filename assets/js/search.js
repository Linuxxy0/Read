...
async function initSearchPage() {
  ...
  let mode = getQueryParam('mode') || 'all';
  if (input) input.value = query;

  function render() {
    const results = searchBooks(books, query).filter((item) => mode === 'all' || item.type === mode);
    if (countEl) countEl.textContent = query ? `“${query}” 共找到 ${results.length} 条结果` : '输入关键词后，即可在全站范围检索作品、章节与正文片段。';
    if (!query) {
      resultsEl.innerHTML = createEmptyState('从一个词开始', '你可以搜索书名、作者、标签、章节名，或者正文中的关键词。');
      return;
    }
    resultsEl.innerHTML = results.length ? results.map((item) => `
      <article class="result-card">
        ...
        <h3 class="result-title"><a href="${item.href}">${highlightText(item.title, query)}</a></h3>
        <p class="result-snippet">${highlightText(item.snippet, query)}</p>
        <div class="result-actions">
          <a class="inline-button primary-button" href="${item.href}">打开结果</a>
        </div>
      </article>
    `).join('') : createEmptyState('没有找到结果', '你可以尝试更短的关键词，或者改搜作者、标签与章节名。');
