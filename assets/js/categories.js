...
async function initCategories() {
  ...
  categoryEl.innerHTML = getAllCategories(books).map((item) => `
    <article class="category-card">
      <div class="count-pill">${item.count} 部作品</div>
      <h3 style="margin:16px 0 0; font-size:1.32rem;">${escapeHtml(item.name)}</h3>
  ...
  `).join('');
  tagEl.innerHTML = getAllTags(books).map((item) => `
    <article class="tag-card">
      <div class="count-pill">${item.count} 次出现</div>
      <h3 style="margin:16px 0 0; font-size:1.12rem;"># ${escapeHtml(item.name)}</h3>
